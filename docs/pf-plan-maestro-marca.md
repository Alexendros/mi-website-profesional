# PRE-FASE — Plan Maestro de Marca Personal Alexendros

> **Estado:** ACTIVO — Abril 2026
> 

> **Objetivo:** Consolidar `alexendros.me` como hub de credibilidad enterprise con biografía real, experiencia verificable, formación documentada, proyectos con impacto y plataforma `alexendros.pro` en construcción (producto paraguas pendiente de reformular).
> 

> **Herramienta:** Claude Code + MCPs · Fuente de verdad para el agente `brand-auditor`
> 

---

## Mapa de subpáginas

| # | Subpágina | Propósito |
| --- | --- | --- |
| PF-0 | 👤 Biografía & Posicionamiento | Historia, UVP, Brand Positioning Statement |
| PF-1 | 💼 Experiencia Profesional | Timeline verificable, impacto cuantificado |
| PF-2 | 🎓 Formación: Certificada & Autodidacta | Títulos, certificaciones, stack autodidacta |
| PF-3 | 🗂️ Repositorios & Proyectos | GitHub, demos, impact metrics |
| PF-4 | 🗄️ Producto paraguas (archivado) | Contenido en `.archive/kitos-pre-reformulacion-2026-04-23/`; pendiente reformular |
| PF-5 | 🎨 Sistema de Identidad Visual | Tokens, logos, paletas, activos |
| PF-6 | ⚡ CWV + GEO + JSON-LD | Técnico: rendimiento y citabilidad AI |
| PF-7 | ✅ Checklist de Cierre & KPIs | Criterios de éxito medibles |

---

## Instrucción para Claude Code

```bash
# Al ejecutar cualquier tarea de marca personal:
# 1. Leer esta página como contexto maestro
# 2. Ir a la subpágina correspondiente para datos específicos
# 3. Activar agente brand-auditor para auditorías
# 4. Publicar cambios en Notion vía MCP antes de modificar código
```

## Regla de coherencia narrativa

Todo texto público de `alexendros.me` debe pasar el **test de los 5 segundos**:

> ¿Un visitante nuevo entiende quién es Alejandro, qué construye y para quién, en menos de 5 segundos?
> 

Si la respuesta es NO → reescribir antes de desplegar.

[PRE-FASE — Plan Maestro de Marca Personal Alexendros](PRE-FASE%20%E2%80%94%20Plan%20Maestro%20de%20Marca%20Personal%20Alexendr%203397ded224cb8189a9bae5bd222bb618.md)

[PF-0 — Biografía & Posicionamiento](PF-0%20%E2%80%94%20Biograf%C3%ADa%20&%20Posicionamiento%203397ded224cb81c088bfedab0e47f112.md)

[PF-1 — Experiencia Profesional](PF-1%20%E2%80%94%20Experiencia%20Profesional%203397ded224cb81db9a7ce85a755a6b92.md)

[PF-2 — Formación: Certificada & Autodidacta](PF-2%20%E2%80%94%20Formaci%C3%B3n%20Certificada%20&%20Autodidacta%203397ded224cb81dd9b5fe7692b274f63.md)

[PF-3 — Repositorios & Proyectos](PF-3%20%E2%80%94%20Repositorios%20&%20Proyectos%203397ded224cb81db9eb0d7b6966824aa.md)

*(PF-4 — definición de producto paraguas — archivado 2026-04-23 en `.archive/kitos-pre-reformulacion-2026-04-23/`; pendiente reformular.)*